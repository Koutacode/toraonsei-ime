...
def download_sources(download_dir: Path) -> list[Path]:
    paths: list[Path] = []
...
def write_tsv(mapping: OrderedDict[str, list[str]], output: Path) -> None:
    output.parent.mkdir(parents=True, exist_ok=True)
    generated_at = datetime.now(timezone.utc).astimezone().strftime("%Y-%m-%d")
    with output.open("w", encoding="utf-8", newline="\n") as w:
        w.write("# source: SKK dictionaries (L, propernoun, jinmei, station)\n")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="skk_dict_") as tmp:
        source_paths = download_sources(Path(tmp))
