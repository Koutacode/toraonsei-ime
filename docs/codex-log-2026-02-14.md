...
## 追加更新（2026-02-15 02:10 JST）
- Gemini修正版の状態で `.\gradlew.bat clean assembleDebug` を実行し、デバッグAPKを再生成。
- `output/toraonsei-debug.apk` を更新（`sha256:7ea4b2f05ce82e65ce5ec87c9d437e0a9806e374a339ab9a84083da7486d8e24`）。
- 既存アセット名の衝突を避けるため、`gh release upload android-debug-20260214 output\\toraonsei-debug.apk --clobber --repo Koutacode/toraonsei-ime` で上書き再アップロード。
- リリース反映を確認（`updatedAt: 2026-02-14T17:09:59Z`）。

## 追加更新（2026-02-15 03:02 JST）
- 上側 `変`（文章整形）実行時の過剰漢字化を抑えるガードを追加（文脈整形の急変候補を自動で却下）。
- 下側 `変換` 候補の過変換抑制ロジックを維持しつつ、`README.md` からカタカナ変換表記を削除してUI仕様と一致させた。
- `.\gradlew.bat assembleDebug testDebugUnitTest` 実行でビルド/テスト通過を確認。
- `output/toraonsei-debug.apk` を更新（`sha256:48e417c7c071558e5c921ae5568ac3dd5679cecebbce40408983cd58612c226a`）。
- `gh release upload android-debug-20260214 output\\toraonsei-debug.apk --clobber --repo Koutacode/toraonsei-ime` で上書き再アップロード。
- リリース反映を確認（`updatedAt: 2026-02-14T18:02:14Z`）。

## 追加更新（2026-02-15 03:05 JST）
- `変` 実行時にローカルLLM整形を常時試行するよう変更（強度設定に関係なく、失敗時は既存ローカル整形へフォールバック）。
- ローカルLLMのプロンプトを用途別に強化（会話=砕けた文体、仕事=敬語/丁寧語）し、不要なカタカナ化・不自然な漢字化を抑止する制約を追加。
- 句読点（、。）、`？`、`！`、長音「ー」の後処理正規化を追加し、日本語出力の安定性を向上。
- `.\gradlew.bat assembleDebug testDebugUnitTest` 実行でビルド/テスト通過を確認。
- `output/toraonsei-debug.apk` を更新（`sha256:5afc4c741e0b788647cf5d4fe1effa4be939894c4ac2e3c9a09217f33e7f03c2`）。
- `gh release upload android-debug-20260214 output\\toraonsei-debug.apk --clobber --repo Koutacode/toraonsei-ime` で上書き再アップロード。
- リリース反映を確認（`updatedAt: 2026-02-14T18:05:40Z`）。

## 追加更新（2026-02-15 03:07 JST）
- `カナ変換` 機能の残骸を整理するため、未使用の `LocalFormatter.toKatakanaSurface` を削除。
- `.\gradlew.bat assembleDebug testDebugUnitTest` 実行でビルド/テスト通過を再確認。
- `output/toraonsei-debug.apk` を更新（`sha256:f956791958df0867fe599f10b302fa70221297f3bc92b506a7cb60e394eb24fa`）。
- `gh release upload android-debug-20260214 output\\toraonsei-debug.apk --clobber --repo Koutacode/toraonsei-ime` で上書き再アップロード。
- リリース反映を確認（`updatedAt: 2026-02-14T18:07:45Z`）。
