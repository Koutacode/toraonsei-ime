...
## 最終更新
- 更新日時: 2026-02-15 04:44 (JST)
- 最新APK: `output/toraonsei-debug.apk`
- ビルド: `.\gradlew.bat clean assembleDebug`
- 更新内容: `英` をローカルLLM翻訳に切り替え、長文はチャンク分割して全文翻訳するよう改善。`英/変` の変換対象抽出範囲を拡大。`かな変換` は廃止し、キーボードの `整形` キーで現在の整形（`変/英`）を適用。`android-debug-20260214` の `toraonsei-debug.apk` を上書き再アップロード（`sha256:0a47b20a1779633fcd2afeaf9dc8c7b72ec7e14cf86c8b0f14f81293dc98346d`）
